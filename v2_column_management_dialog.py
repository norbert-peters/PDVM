"""
MODAL-DIALOG: SPALTEN VERWALTEN V2
===================================

Blockierendes Fenster für Spalten-Management mit aktueller Sicht
V2 Version mit korrektem GCS-Import
"""
...
class ColumnManagementDialog(QDialog):
    ...
    def __init__(self, parent, view_guid, controls_config, current_mode="table"):
        ...
        logger.info(f"🏗️ Spalten-Management Dialog geöffnet (Mode: {current_mode}, Expert: {self.gcs.expert_mode})")
    
    def setup_ui(self):
        """Erstelle die Dialog-UI"""
        # Dialog-Eigenschaften
        mode_name = self.current_mode.title()
        expert_label = "ExpertMode" if self.gcs.expert_mode else "StandardMode"
    ...
    def load_current_columns(self):
        ...
        try:
            ...
            all_controls, _ = self.gcs.db.get_value(self.view_guid, 'controls')
            if not all_controls:
            ...
            projection_key = 'change_expert' if self.gcs.expert_mode else 'change_standard'
            current_projection = self.gcs.get_projection_table(self.view_guid, projection_key)
    ...
    def accept_changes(self):
        ...
        try:
            ...
            for row in range(self.columns_list.count()):
                ...
                if expert_checkbox and self.gcs.expert_mode:
            ...
            if changes_made or len(new_order) > 0:  # Auch bei Sortierung ohne Property-Änderung
                ...
                self.gcs.db.set_value(self.view_guid, 'controls', all_controls)
