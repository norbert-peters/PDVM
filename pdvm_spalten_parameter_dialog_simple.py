...
class PdvmSpaltenParameterDialog(QDialog):
    ...
    def _load_from_provider(self):
        """Lädt Controls vom Provider und bereitet display_* Felder vor"""
        try:
            # Schritt 1: Controls vom Provider holen
            controls_result = self.daten_manager.get_current_controls(for_dialog=True)
            if not controls_result or not hasattr(controls_result, 'columns'):
                logger.error("❌ Keine Controls vom Provider erhalten")
                self.working_columns = []
                return
                
            all_columns = controls_result.columns
            logger.info(f"📥 {len(all_columns)} Controls vom Provider geladen")
            
            # Schritt 2: Für Dialog vorbereiten - display_* Felder setzen
            self.working_columns = []
            for col in all_columns:
                # Kopie erstellen
                display_col = col.copy()
                
                # Display-Felder basierend auf Mode setzen
                if self.mode == "normal":
                    # Normal-Mode: show_show und show_order verwenden
                    display_col['display_show'] = col.get('show_show', True)
                    display_col['display_order'] = col.get('show_order', 999)
                    display_col['display_expert'] = col.get('expert', False)
                else:
                    # Expert-Mode: show und order verwenden
                    display_col['display_show'] = col.get('show', True)  
                    display_col['display_order'] = col.get('order', 999)
                    display_col['display_expert'] = col.get('expert', False)
                
                # Im Normal-Mode nur non-expert Spalten anzeigen
                if self.mode == "normal" and display_col['display_expert']:
                    continue
            ...
            logger.error(f"❌ Fehler beim Laden der Provider-Controls: {e}")
            self.working_columns = []
    ...
    def _apply_changes(self):
        ...
        try:
            ...
            # Schritt 4: Über Daten-Manager persistieren und Tabelle neu laden
    ...
    def _write_back_to_provider(self):
        """Schreibt display_* Werte zurück in die Provider-Controls"""
        try:
            # Über den Daten-Manager die aktuellen Controls holen
            current_controls = self.daten_manager.get_current_controls(for_dialog=False)
            if not current_controls or not hasattr(current_controls, 'columns'):
                raise Exception("Keine aktuellen Controls verfügbar")
            
            # Änderungen zurückschreiben
            provider_columns = current_controls.columns
            changes_made = 0
            
            for display_col in self.working_columns:
                col_name = display_col.get('name')
                if not col_name:
                    continue
                    
                # Entsprechende Provider-Spalte finden
                for provider_col in provider_columns:
                    if provider_col.get('name') == col_name:
                        if self.mode == "normal":
                            # Normal-Mode: display_* → show_*
                            provider_col['show_show'] = display_col.get('display_show', True)
                            provider_col['show_order'] = display_col.get('display_order', 999)
                        else:
                            # Expert-Mode: display_* → show/order  
                            provider_col['show'] = display_col.get('display_show', True)
                            provider_col['order'] = display_col.get('display_order', 999)
                            provider_col['expert'] = display_col.get('display_expert', False)
                        
                        changes_made += 1
                        break
            
            logger.info(f"💾 {changes_made} Spalten zurückgeschrieben in Provider-Controls")
