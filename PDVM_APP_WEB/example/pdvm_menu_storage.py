...
from pdvm_menu_schema import MenuContainer, MenuItem, MenuCommand, MenuItemType
...
class V2MenuStorage:
    ...
    def get_menu(self, menu_guid: str) -> Optional[MenuContainer]:
        ...
        try:
            ...
            # KONVERTIERUNG: V3-Struktur (Dicts) → V2-Struktur (Arrays)
            # V3: {"VERTIKAL": {"guid1": {...}, "guid2": {...}}}
            # V2: {"VERTIKAL": [{...}, {...}], "MENU_NAME": "...", "MENU_GUID": "..."}
            
            converted = {
                'MENU_GUID': menu_guid,
                'MENU_NAME': menu_dict.get('META', {}).get('NAME', 'Unbenanntes Menü'),
                'VERTIKAL': [],
                'GRUND': [],
                'ZUSATZ': [],
                'COMMANDS': [],
            ...
            # Konvertiere Dictionaries zu Arrays
            for section in ['VERTIKAL', 'GRUND', 'ZUSATZ']:
                if section in menu_dict and isinstance(menu_dict[section], dict):
                    # Jedes Item-Dict in Array umwandeln
                    for item_guid, item_data in menu_dict[section].items():
                        # Item-Struktur bereits korrekt (guid, type, label, etc.)
                        converted[section].append(item_data)
            
            container = MenuContainer.from_dict(converted)
...
if __name__ == "__main__":
    ...
    from pdvm_menu_schema import create_menu_item, create_command
