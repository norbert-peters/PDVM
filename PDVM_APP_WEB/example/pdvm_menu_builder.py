...
class V2VerticalMenuWidget(QWidget):
    ...
    def set_items(self, items: List[MenuItem], container=None):
        """
        Setzt Menü-Items (mit Submenu-Unterstützung)
        
        Args:
            items: Liste von MenuItem (bereits sortiert, nur Top-Level Items!)
            container: MenuContainer für rekursive Submenüs
        """
        # Clear alte Buttons
        self.clear()
        
        # Speichere Container für rekursive Submenüs
        self.container = container
        
        # Erstelle neue Buttons (NUR Top-Level Items mit parent_guid=None)
        for item in items:
            if not item.VISIBLE:
                continue
            
            # NUR Top-Level Items (PARENT_GUID ist None)
            if item.PARENT_GUID is not None:
                continue
            ...
            if item.TYPE == MenuItemType.SEPARATOR:
                ...
                # NEU: Submenu-Button mit Dropdown
    ...
    def _add_submenu_button(self, item: MenuItem, container):
        """
        Fügt Submenu-Button hinzu (mit Dropdown)
        
        Args:
            item: MenuItem vom Typ SUBMENU
            container: MenuContainer für rekursive Submenüs
        """
        # Erstelle Button mit Dropdown-Pfeil
        button = QPushButton(item.LABEL)
        button.setFixedHeight(40)
        
        # Styling (ähnlich wie V2MenuButton)
        ...
        # Erstelle Dropdown-Menü (REKURSIV)
        menu = self._create_submenu_recursive(item.GUID, container)
        button.setMenu(menu)
    ...
    def _create_submenu_recursive(self, parent_guid: str, container) -> QMenu:
        """
        Rekursive Erstellung von Submenüs (identisch zu Horizontal-Menü)
        
        Args:
            parent_guid: GUID des Parent-Items
            container: MenuContainer mit allen Menüdaten
            
        Returns:
            QMenu mit rekursiv erstellten Submenüs
        """
        menu = QMenu()
        
        print(f"🔍 DEBUG (VERTIKAL) _create_submenu_recursive: parent_guid={parent_guid}")
        
        if not container:
            print(f"❌ DEBUG (VERTIKAL): Container ist None!")
            return menu
        
        # Hole Child-Items aus Container
        child_items = [
            child for child in container.GRUND + container.VERTIKAL
            if child.PARENT_GUID == parent_guid and child.VISIBLE
        ]
        child_items.sort(key=lambda x: x.SORT_ORDER)
        print(f"📊 DEBUG (VERTIKAL): {len(child_items)} Kinder gefunden für parent_guid={parent_guid}")
        
        for child in child_items:
            print(f"  🔹 (VERTIKAL) Child: {child.LABEL} | TYPE={child.TYPE} | TYPE.value={child.TYPE.value}")
            if child.TYPE == MenuItemType.SEPARATOR:
                # Separator
                print(f"    ➡️ (VERTIKAL) SEPARATOR hinzugefügt")
                menu.addSeparator()
                
            elif child.TYPE == MenuItemType.SUBMENU:
                # REKURSIV: Erstelle Unter-Submenü
                print(f"    ➡️ (VERTIKAL) SUBMENU erkannt! Rekursiver Aufruf für GUID={child.GUID}")
                submenu = self._create_submenu_recursive(child.GUID, container)
                ...
                # BUTTON: Normale Action
                print(f"    ➡️ (VERTIKAL) BUTTON hinzugefügt")
...
class V2HorizontalMenuWidget(QWidget):
    ...
    def _add_submenu_button(self, item: MenuItem, container: Optional[MenuContainer]):
        """Fügt Button mit Submenü hinzu (REKURSIV für n Ebenen)"""
        ...
        # Erstelle Submenü REKURSIV
        submenu = self._create_submenu_recursive(item.GUID, container)
    ...
    def _create_submenu_recursive(self, parent_guid: str, container: Optional[MenuContainer]) -> QMenu:
        """
        Erstellt Submenü REKURSIV für n Ebenen
        
        Logik:
        - Hole alle Child-Items von parent_guid
        - Für jedes Kind:
          - BUTTON/SEPARATOR → QAction
          - SUBMENU → QMenu (rekursiv!)
        """
        menu = QMenu()
        
        print(f"🔍 DEBUG _create_submenu_recursive: parent_guid={parent_guid}")
        
        if not container:
            print(f"❌ DEBUG: Container ist None!")
            return menu
        ...
        child_items.sort(key=lambda x: x.SORT_ORDER)
        print(f"📊 DEBUG: {len(child_items)} Kinder gefunden für parent_guid={parent_guid}")
        
        for child in child_items:
            print(f"  🔹 Child: {child.LABEL} | TYPE={child.TYPE} | TYPE.value={child.TYPE.value}")
            if child.TYPE == MenuItemType.SEPARATOR:
                # Separator
                print(f"    ➡️ SEPARATOR hinzugefügt")
                menu.addSeparator()
                
            elif child.TYPE == MenuItemType.SUBMENU:
                # REKURSIV: Erstelle Unter-Submenü
                print(f"    ➡️ SUBMENU erkannt! Rekursiver Aufruf für GUID={child.GUID}")
                submenu = self._create_submenu_recursive(child.GUID, container)
                submenu_action = menu.addMenu(submenu)
                submenu_action.setText(child.LABEL)
...
class V2MenuBuilder:
    ...
    def build_vertical(
        ...
        """
        try:
            # Lade kompletten Container (für rekursive Submenüs)
            container = self.storage.get_menu(menu_guid)
            if not container:
                logger.warning(f"⚠️ Container nicht gefunden für {menu_guid}")
                return None
            
            items = container.VERTIKAL
            if not items:
                logger.warning(f"⚠️ Keine VERTIKAL Items für {menu_guid}")
                return None
            
            # Erstelle Widget (mit Container für Submenüs!)
