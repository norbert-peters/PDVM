...
class PdvmSpaltenParameterDialog(QDialog):
    ...
    def _init_data(self):
        ...
        if not self.controls:
            logger.warning("⚠️ Keine Controls-Struktur vorhanden")
            self.working_columns = []
            return
            
        # ControlsResult Objekt hat ein 'columns' Attribut
        if hasattr(self.controls, 'columns'):
            all_columns = self.controls.columns
        elif isinstance(self.controls, dict) and 'columns' in self.controls:
    ...
    def _apply_changes(self):
        ...
        try:
            ...
            # Im Normal-Mode: Order aus aktueller Position übernehmen
            if self.mode == "normal":
                logger.info(f"💾 Normal-Mode: Übernehme Order aus aktueller Position...")
                self._update_order_from_position()
    ...
    def _update_order_from_position(self):
        """Übernimmt Order-Werte aus der aktuellen Tabellenposition (Normal-Mode)"""
        logger.info(f"🔄 Aktualisiere show_order basierend auf aktueller Position...")
        for row in range(len(self.working_columns)):
            old_order = self.working_columns[row].get('show_order', 0)
            new_order = row + 1
            self.working_columns[row]['show_order'] = new_order
            logger.debug(f"   Spalte {self.working_columns[row].get('name', 'unknown')}: {old_order} -> {new_order}")
        logger.info(f"✅ Order-Werte für {len(self.working_columns)} Spalten aktualisiert")
        ...
        try:
            ...
            for row in range(len(self.working_columns)):
                ...
                if self.mode == "expert":
                    ...
                    if expert_item:
                        old_expert = column.get('expert', False)
                        new_expert = expert_item.checkState() == Qt.Checked
                        column['expert'] = new_expert
                        if old_expert != new_expert:
                            logger.info(f"  📝 {column_name}: expert {old_expert} -> {new_expert}")
                    col_index += 1
                
                # Show flag
                show_item = self.table.item(row, col_index)
                if show_item:
                    old_show = column.get('show', True)
                    new_show = show_item.checkState() == Qt.Checked
                    column['show'] = new_show
                    if old_show != new_show:
                        logger.info(f"  📝 {column_name}: show {old_show} -> {new_show}")
                col_index += 1
                
                # Order ist bereits durch _update_order_from_position() (Normal) oder _update_order_from_table() (Expert) gesetzt
