...
class PdvmViewDatenManager:
                ...
                if not col_type.startswith("date_"):
                    ...
                    if feld and gruppe:
                        try:
                            ...
                            # Nur Wert anzeigen, falls Dict: nur 'wert', falls leeres Dict: leer
                            ...
                            logger.debug(f"✅ {col_name} = {row_record[col_name]} (get_value)")
                        except Exception as e:
    ...
    def get_table_data_for_display(self, show_only=True):
        """Gibt ALLE Spalten (inkl. Dummy) in der Reihenfolge der Controls zurück, ohne Filterung. Leere Zeilen werden entfernt."""
        try:
