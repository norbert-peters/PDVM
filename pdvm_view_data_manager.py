...
class PdvmViewDataManager:
    ...
    def _load_view_configuration(self):
        ...
        try:
            ...
            # NEUE STRUKTUR: Tabellennamen aus ROOT extrahieren
            ...
            # NEUE STRUKTUR: Feld-Konfigurationen aus metadata extrahieren
            ...
            # VIEW PARAMETER: show_YMD und show_alter aus ROOT-Konfiguration laden
            self.show_YMD = root_config.get("show_YMD", False)
            self.show_alter = root_config.get("show_alter", False)
            
            if not self.fields_config:
                raise ValueError(f"Keine Feld-Konfigurationen in metadata.{self.table_name}.felder gefunden")
            
            logger.info(f"✅ View-Konfiguration geladen: {self.table_name} mit {len(self.fields_config)} Feldern")
            logger.info(f"📊 View-Parameter: show_YMD={self.show_YMD}, show_alter={self.show_alter}")
    ...
    def _add_display_data(self):
        """
        Fügt zusätzliche Display-Informationen zu den Daten hinzu und konvertiert Raw-Keys in Display-Werte.
        KRITISCH: Dropdown-Keys werden in menschenlesbare Werte übersetzt.
        """
        try:
            processed_data = []
            ...
            for record in self.processed_data:
                ...
                # Für jedes Dropdown-Feld: Raw-Key in Display-Wert konvertieren
                for field_config in self.fields_config:
                    ...
                    if field_type == "dropdown":
                        ...
                        if raw_value is not None:
                            # Raw-Wert für Filter-Zugriff speichern
                            processed_record[f"_raw_{field_name}"] = raw_value
                            
                            # Display-Wert für UI-Anzeige generieren
                            display_value = self.get_display_value(field_name, raw_value)
                            processed_record[field_name] = display_value
                            
                            logger.debug(f"🔄 {field_name}: '{raw_value}' → '{display_value}'")
                    
                    elif field_type == "date":
                        # Für Date-Felder: Automatisch Jahr/Monat/Tag-Spalten erstellen
                        date_value = record.get(field_name)
                        
                        # IMMER interne Spalten für Filter-Zugriff erstellen (auch bei leeren Werten)
                        if date_value is not None and str(date_value).strip():
                            try:
                                from pd_datetime import Pdvm_DateTime
                                pdvm_dt = Pdvm_DateTime("DEU")
                                pdvm_dt.PdvmDateTime = float(date_value)
                                
                                # Interne Spalten für Filter-Zugriff erstellen
                                ...
                                # Show_YMD: Zusätzliche sichtbare Spalten für Jahr/Monat/Tag
                                if self.show_YMD:
                                ...
                                # Show_Alter: Altersberechnung zum aktuellen Datum
                                ...
                                logger.debug(f"🗓️ {field_name}: {date_value} → Jahr={pdvm_dt.Year}, Monat={pdvm_dt.Month}, Tag={pdvm_dt.Day}")
                                
                            except Exception as e:
                                logger.warning(f"⚠️ Fehler beim Verarbeiten des Datums {field_name}={date_value}: {e}")
                                # Bei Fehlern: Spalten mit None-Werten erstellen
                            ...
                            # Leere Datumswerte: Interne Spalten mit None erstellen
                            processed_record[f"_{field_name}_year"] = None
                            processed_record[f"_{field_name}_month"] = None
                            processed_record[f"_{field_name}_day"] = None
            ...
            # Verarbeitete Daten ersetzen
            self.processed_data = processed_data
            
            logger.debug(f"🔍 Display-Daten aufbereitet für {len(self.processed_data)} Datensätze")
            logger.info(f"📋 Dropdown-Werte übersetzt: {self.get_dropdown_fields()}")
            
        except Exception as e:
            logger.error(f"❌ Fehler bei Display-Daten-Aufbereitung: {e}")
            # Bei Fehler: Originaldaten beibehalten
    
    def _calculate_age(self, geburtsdatum_dt):
        """
    ...
    def get_fields_config(self) -> List[Dict[str, Any]]:
        """
        Gibt die Feld-Konfigurationen zurück, erweitert um dynamische YMD- und Alter-Spalten.
        
        Returns:
            Liste der Feld-Konfigurationen
        """
        extended_config = self.fields_config.copy()
        
        # ERWEITERT: Zusätzliche Spalten für show_YMD und show_alter hinzufügen
        for field_config in self.fields_config:
            field_name = field_config.get("feld")
            field_type = field_config.get("type")
            
            if field_type == "date" and field_name:
                # Show_YMD: Jahr/Monat/Tag-Spalten hinzufügen
                if self.show_YMD:
                    extended_config.extend([
                        {
                            "feld": f"{field_name}_Jahr",
                            "titel": f"{field_config.get('titel', field_name)} Jahr",
                            "type": "int",
                            "sortable": True,
                            "width": 60,
                            "ui": {"filterType": "text"}
                        ...
                        {
                            "feld": f"{field_name}_Monat",
                            "titel": f"{field_config.get('titel', field_name)} Monat",
                            "type": "int",
                            "sortable": True,
                            "width": 60,
                            "ui": {"filterType": "text"}
                        ...
                        {
                            "feld": f"{field_name}_Tag",
                            "titel": f"{field_config.get('titel', field_name)} Tag",
                            "type": "int",
                            "sortable": True,
                            "width": 50,
                            "ui": {"filterType": "text"}
                ...
                # Show_Alter: Alter-Spalte hinzufügen (nur für Geburtsdatum)
                if self.show_alter and field_name.lower() in ['geburtsdatum', 'geburtstag']:
                    extended_config.append({
                        "feld": f"{field_name}_Alter",
                        "titel": "Alter",
                        "type": "int",
                        "sortable": True,
                        "width": 50,
    ...
    def get_field_config(self, field_name: str) -> Dict[str, Any]:
        """
        Gibt die Konfiguration für ein spezifisches Feld zurück.
        
        Args:
            field_name: Name des Feldes
            
        Returns:
            Feld-Konfiguration oder leeres Dict wenn nicht gefunden
        """
        for field_config in self.fields_config:
    ...
    def get_field_name_by_index(self, index: int) -> Optional[str]:
        """
        Gibt den Feldnamen für einen gegebenen Index zurück.
        
        Args:
            index: Index des Feldes in der Konfiguration
            
        Returns:
            Feldname oder None wenn Index außerhalb des Bereichs
        """
        if 0 <= index < len(self.fields_config):
            return self.fields_config[index].get("feld")
        return None
    ...
    def get_display_value(self, field_name: str, raw_value: Any) -> str:
        ...
        try:
            ...
            # DATUMS-FELDER: PdvmDateTime.Date verwenden
            elif field_config and field_config.get("type") == "date":
                try:
                    from pd_datetime import Pdvm_DateTime
                    pdvm_dt = Pdvm_DateTime("DEU")
                    pdvm_dt.PdvmDateTime = float(raw_value)
                    return pdvm_dt.Date  # Formatiertes Datum zurückgeben
                except Exception as e:
                    logger.error(f"❌ Fehler beim Formatieren des Datums {raw_value}: {e}")
    ...
    def apply_filters(self, filters: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        Wendet Filter auf die Daten an und gibt gefilterte Daten zurück.
        ...
        """
        if not filters:
            logger.debug("🔍 Keine Filter angewendet - alle Daten zurückgegeben")
            return self.processed_data
        ...
        try:
            ...
            logger.info(f"🔍 Filter angewendet: {len(filtered_data)} von {len(self.processed_data)} Datensätzen gefunden")
            logger.debug(f"🔍 Angewendete Filter: {filters}")
    ...
    def clear_filters(self) -> List[Dict[str, Any]]:
        """
        Entfernt alle Filter und gibt alle Daten zurück.
        
        Returns:
            Liste aller Datensätze
        """
        logger.info("🔄 Alle Filter entfernt - alle Daten angezeigt")
        return self.processed_data
