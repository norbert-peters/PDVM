...
class PdvmCentralDatenbank:
    ...
    def _lesen_rogue(self, guid: str) -> Optional[Any]:
        ...
        Mit automatischer Reparatur von JSON-Doppel-Encoding.
        """
        conn = sqlite3.connect(self.db_name)
        cursor = conn.cursor()
        ...
        if row:
            ...
            if isinstance(row[0], str):
                ...
                if '\\\"' in raw_data:
                    escaped_count = raw_data.count('\\\"')
                    logger.info(f"[AUTOREPAIR-CENTRAL] GEFUNDEN: {escaped_count} escaped quotes in {self.table_name} für GUID {guid}")
                    
                    # Repariere den rohen JSON-String
                    repaired_raw = self._auto_repair_json_string(raw_data)
                    if repaired_raw != raw_data:
                        logger.info(f"[AUTOREPAIR-CENTRAL] Reparatur erfolgreich - aktualisiere DB automatisch")
                        ...
                        try:
                            ...
                            conn2.commit()
                            conn2.close()
                            logger.info(f"[AUTOREPAIR-CENTRAL] DB erfolgreich aktualisiert")
                        except Exception as repair_error:
                            logger.error(f"[AUTOREPAIR-CENTRAL] DB-Update-Fehler: {repair_error}")
                        
                        raw_data = repaired_raw
                
                try:
                    return json.loads(raw_data)  # Versuche, es als JSON zu parsen
                except json.JSONDecodeError:
                    logger.error(f"JSONDecodeError beim Parsen von {raw_data}")
                    return None
            return row[0]
    ...
    def _auto_repair_json_string(self, damaged_json: str) -> str:
        ...
        """
        logger.info(f"[AUTOREPAIR-CENTRAL] Repariere JSON-String ({len(damaged_json)} Zeichen)...")
        ...
        try:
            json.loads(repaired)
            logger.info(f"[AUTOREPAIR-CENTRAL] Reparatur erfolgreich - gültiges JSON erzeugt")
            return repaired
        except:
            logger.info(f"[AUTOREPAIR-CENTRAL] Reparatur teilweise - kein gültiges JSON, aber escaped quotes entfernt")
            return repaired
    ...
    def get_value(self, gruppe: str, feld: str, ab_zeit: Optional[float] = None) -> Dict[str, Any]:
        ...
        if self.historisch:
            ...
            # NICHT-historisch: raw_val ist der direkte Wert
            # WICHTIG: Nicht als historisches Dict interpretieren!
            return {"wert": raw_val, "ab_zeit": 0.0}

    def get_value_no_json(self, gruppe: str, feld: str, ab_zeit: Optional[float] = None) -> Dict[str, Any]:
        """
        Wie get_value(), aber OHNE automatisches JSON-Parsing.
        Rohe Strings bleiben als Strings erhalten.
        
        Args:
            gruppe: Name der Gruppe
            feld: Name des Feldes  
            ab_zeit: Zeitstempel (nur bei historischen Daten relevant)
            
        Returns:
            Dict mit {"wert": ..., "ab_zeit": ...} - identisch zu get_value()
        """
        if not self.data:
            return {}
        ...
        raw_val = grp_dict.get(feld)
        if raw_val is None:
            logger.debug(f"→ get_value_no_json: Feld '{feld}' nicht vorhanden in Gruppe '{gruppe}'.")
            return {}

        # KEIN automatisches JSON-Parsing! raw_val bleibt wie es ist.
        ...
        if self.historisch:
            ...
            if not isinstance(raw_val, dict):
                # Sollte nicht vorkommen, aber defensiv abfangen
                logger.warning(f"→ get_value_no_json: Erwartete Dict (historisch), bekam {type(raw_val)}; gebe als Skalar zurück.")
                return {"wert": raw_val, "ab_zeit": ab_zeit}
    ...
    def save_values(self):
        ...
        INTELLIGENTE JSON-BEHANDLUNG: Verwendet speichern_no_json für bereits JSON-kodierte Strukturen.
        """
        ...
        # === PRÜFE OB DIREKTER JSON-STRING GESPEICHERT WERDEN SOLL ===
        # Falls nur eine Gruppe existiert und diese nur JSON-String Felder hat, 
        # verwende direkten JSON-String ohne weitere Verarbeitung
        if self._should_use_direct_json_storage(clean_data):
            # Erstelle JSON-String direkt aus der Struktur OHNE weitere JSON-Kodierung der Inhalte
            json_string = self._create_direct_json_string(clean_data)
            print(f"[DEBUG] save_values: Verwende DIREKTE JSON-Speicherung")
            print(f"[DEBUG] save_values: JSON Length: {len(json_string)}")
            print(f"[DEBUG] save_values: Kein Doppel-JSON-Encoding der String-Inhalte")
            
            from pdvm_datenbank import PdvmDatenbank
            data_db = PdvmDatenbank(db_name=self.db_name, table_name=self.table_name)
            data_db.speichern_no_json(self.guid, json_string)
        else:
            # Standard-Speicherung für gemischte Strukturen
            print(f"[DEBUG] save_values: Verwende Standard-Speicherung")
    ...
    def _should_use_direct_json_storage(self, data):
        """
        Prüft ob direkte JSON-Speicherung verwendet werden soll.
        Kriterium: Struktur enthält nur JSON-String Felder (von set_raw_string erstellt).
        """
        if not isinstance(data, dict):
            return False
            
        for gruppe_name, gruppe_data in data.items():
            if not isinstance(gruppe_data, dict):
                continue
                
            # Prüfe ob alle Felder in der Gruppe JSON-Strings sind
            for feld_name, feld_wert in gruppe_data.items():
                if isinstance(feld_wert, str):
                    # Prüfe ob es gültiges JSON ist
                    try:
                        json.loads(feld_wert)
                        continue
                    except:
                        return False
                else:
                    # Nicht-String Werte → keine direkte JSON-Speicherung
                    return False
        
        return True

    def _create_direct_json_string(self, data):
        """
        Erstellt JSON-String OHNE Doppel-Encoding der bereits JSON-kodierten String-Inhalte.
        Die String-Werte werden als Raw-Strings behandelt, nicht als zu-JSON-kodierende Objekte.
        """
        print(f"[DEBUG] _create_direct_json_string: Input data = {data}")
        
        # Erstelle JSON manuell um Doppel-Encoding zu vermeiden
        json_parts = []
        json_parts.append('{')
        
        gruppe_items = []
        for gruppe_name, gruppe_data in data.items():
            print(f"[DEBUG] Processing gruppe: {gruppe_name} = {type(gruppe_data)} = {gruppe_data}")
            if isinstance(gruppe_data, dict):
                feld_items = []
                for feld_name, feld_wert in gruppe_data.items():
                    print(f"[DEBUG] Processing feld: {feld_name} = {type(feld_wert)}")
                    if isinstance(feld_wert, str):
                        # String-Wert als RAW übernehmen (bereits JSON-kodiert)
                        print(f"[DEBUG] Using RAW string for {feld_name}")
                        feld_items.append(f'"{feld_name}":{feld_wert}')
                    else:
                        # Normales JSON-Encoding für Nicht-Strings
                        print(f"[DEBUG] Using JSON encoding for {feld_name}")
                        feld_items.append(f'"{feld_name}":{json.dumps(feld_wert)}')
                
                gruppe_json = '{' + ','.join(feld_items) + '}'
                gruppe_items.append(f'"{gruppe_name}":{gruppe_json}')
        
        json_parts.append(','.join(gruppe_items))
        json_parts.append('}')
        
        result = ''.join(json_parts)
        print(f"[DEBUG] _create_direct_json_string: Result = {result[:200]}...")
        return result
    ...
    def set_value_no_json(self, gruppe: str, feld: str, wert: Any, ab_zeit: Optional[float] = None):
        """
        Wie set_value(), aber für rohe Strings gedacht - verhindert automatisches JSON-Parsing.
        Identische API und Verhalten zu set_value().
        
        Args:
            gruppe: Name der Gruppe
            feld: Name des Feldes  
            wert: Der Wert (bleibt als String erhalten, wird NICHT als JSON geparst)
            ab_zeit: Zeitstempel (nur bei historischen Daten relevant)
        """
        # Exakt dieselbe Logik wie set_value(), da das JSON-Parsing beim Lesen passiert
        self.set_value(gruppe, feld, wert, ab_zeit)

    def set_raw_string(self, gruppe: str, feld: str, raw_string: str):
        """
        Direkte String-Speicherung ohne jegliche JSON-Verarbeitung.
        Für bereits serialisierte JSON-Strings gedacht.
        
        Args:
            gruppe: Name der Gruppe
            feld: Name des Feldes
            raw_string: Der rohe String (wird direkt gespeichert)
        """
        print(f"[DEBUG] PdvmCentralDatenbank.set_raw_string: gruppe={gruppe}, feld={feld}, raw_string_length={len(raw_string)}")
        
        # 1) Gruppe anlegen, falls nicht vorhanden
        if gruppe not in self.data or self.data.get(gruppe) is None:
            self.data[gruppe] = {}
        
        # 2) String direkt speichern (kein JSON-Encoding)
        print(f"[DEBUG] PdvmCentralDatenbank.set_raw_string: Direkte Speicherung ohne JSON-Verarbeitung")
        self.data[gruppe][feld] = raw_string
        print(f"[DEBUG] PdvmCentralDatenbank.set_raw_string: Gespeichert self.data[{gruppe}][{feld}]={raw_string[:100]}...")
        
        # Flag setzen für Persistierung
        self.aenderungen_vorhanden = True


    def get_raw_string(self, gruppe: str, feld: str, ab_zeit: Optional[float] = None) -> Optional[str]:
        """
        ENTWICKLUNGSPHASE: Lädt einen rohen String ohne JSON-Parsing
        MIT AUTOMATISCHER BEREINIGUNG von JSON-Doppel-Encoding Problemen.
        
        Args:
            gruppe: Die Gruppe in der Datenbank
            feld: Das Feld in der Gruppe
            ab_zeit: Optional für historische Datenhaltung
            
        Returns:
            Der ursprüngliche rohe String oder None (automatisch repariert wenn nötig)
        """
        result = self.get_value_no_json(gruppe, feld, ab_zeit)
        if result and 'wert' in result:
            raw_string = result['wert']
            print(f"[DEBUG] PdvmCentralDatenbank.get_raw_string: {gruppe}.{feld} -> {len(str(raw_string))} Zeichen")
            
            # === AUTOMATISCHE BEREINIGUNG VON \" PROBLEMEN ===
            if isinstance(raw_string, str) and '\\\"' in raw_string:
                escaped_count = raw_string.count('\\\"')
                print(f"[AUTOREPAIR] GEFUNDEN: {escaped_count} escaped quotes in {gruppe}.{feld}")
                
                # Versuche automatische Reparatur
                repaired_string = self._auto_repair_json_encoding(raw_string)
                if repaired_string != raw_string:
                    print(f"[AUTOREPAIR] Reparatur erfolgreich - speichere bereinigten String")
                    
                    # Speichere die reparierte Version sofort
                    self.set_raw_string(gruppe, feld, repaired_string)
                    self.save_values()
            ...
            print(f"[DEBUG] PdvmCentralDatenbank.get_raw_string: {gruppe}.{feld} -> nicht gefunden")
            return None

    def _auto_repair_json_encoding(self, damaged_string: str) -> str:
        """
        Automatische Reparatur von JSON-Doppel-Encoding Problemen.
        Konvertiert \\\" zurück zu \" und andere escaped Zeichen.
        
        Args:
            damaged_string: String mit JSON-Doppel-Encoding
            
        Returns:
            str: Reparierter String
        """
        print(f"[AUTOREPAIR] Versuche Reparatur von {len(damaged_string)} Zeichen...")
        
        # Schritt 1: \" -> "
        repaired = damaged_string.replace('\\"', '"')
    ...
    def delete_group(self, gruppe: str) -> bool:
        """
        Löscht eine komplette Gruppe aus der Datenstruktur im RAM.
        Dies entfernt alle Felder der angegebenen Gruppe.
        Persistiert wird erst mit save_values().
        ...
        if gruppe in self.data:
            logger.info(f"🗑️ Lösche komplette Gruppe '{gruppe}' mit {len(self.data[gruppe]) if isinstance(self.data[gruppe], dict) else 1} Feldern")
            del self.data[gruppe]
            self._dirty = True  # Markiere als geändert
            return True
        else:
            logger.debug(f"🔍 Gruppe '{gruppe}' nicht gefunden - nichts zu löschen")
            return False
